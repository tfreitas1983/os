...
export default class ResumeAdmin extends Component {
    constructor(props) {
        super(props)
    ...
    pegaChamados(page = 1) {        
        ChamadoDataService.buscarTodos(page)
            ...
            // "docs" com os dados do chamado e "info" com os dados das páginas
                ...
                this.setState({
                    chamados: docs.filter((item) => {return (item.area === this.state.currentUser.area[0] || item.area === this.state.currentUser.area[1] || item.area === this.state.currentUser.area[2] ||item.area === this.state.currentUser.area[3] ||item.area === this.state.currentUser.area[4] )}),
                    info: info,
                    page: page
                })                
            })
    ...
    render () {
        ...
        const {pendentes, finalizados, atrasados, chamados} = this.state 
